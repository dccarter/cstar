...
static void usage() {
    printf(
        "cxy -- a programming language giving c steroids (ver. " CXY_VERSION ")\n"
...
static inline bool checkOptionArg(int i, int argc, char** argv, Log* log) {
    if (i + 1 >= argc) {
        logError(log, NULL, "missing argument for option '{s}'", (FormatArg[]) { { .s = argv[i] } });
        return false;
    }
    return true;
}
...
bool parse_options(int* argc, char** argv, Options* options, Log* log) {
    ...
    for (int i = 1, n = *argc; i < n; ++i) {
        ...
        if (!strcmp(argv[i], "-h") || !strcmp(argv[i], "--help")) {
            ...
            logError(log, NULL, "invalid option '{s}'", (FormatArg[]) { { .s = argv[i] } });
            goto error;
        }
    }
    if (file_count == 0) {
        logError(log, NULL, "no input file, run with '--help' to display usage", NULL);
        goto error;
    }
    goto exit;
