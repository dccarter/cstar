...
#define CXY_ANONYMOUS_FUNC "CXY_Anon_Func"
#define CXY_ANONYMOUS_TUPLE "CXY_Anon_Tuple"
#define CXY_ANONYMOUS_STRUCT "CXY_Anon_Struct"
#define CXY_ANONYMOUS_ARRAY "CXY_Anon_Array"
#define CXY_ANONYMOUS_SLICE "CXY_Anon_Slice"
#define CXY_ANONYMOUS_ENUM "CXT_Anon_Enum"
...
static void epilogue(ConstAstVisitor *visitor, const AstNode *node)
{
    ...
    if (!ctx->importedFile) {
        // @TODO append(ctx->state, CXY_MAIN_CODE, CXY_MAIN_CODE_SIZE);
    }
...
static void prologue(ConstAstVisitor *visitor, const AstNode *node)
{
    ...
    if (!ctx->importedFile) {
        format(ctx->state,
               ...
               NULL);
        format(ctx->state, "\n\n", NULL);
        // @TODO        append(ctx->state, CXY_SETUP_CODE, CXY_SETUP_CODE_SIZE);
