...
https://github.com/tinycthread/tinycthread/blob/master/source/tinycthread.c
*/
...
#ifndef _TINYCTHREAD_ALLOC
#define _TINYCTHREAD_ALLOC malloc
#define _TINYCTHREAD_FREE free
#endif
...
#define thrd_timedout                                                          \
    2 /**< The time specified in the call was reached without acquiring the    \
         requested resource */
#define thrd_busy                                                              \
    3 /**< The requested operation failed because a tesource requested by a    \
         test and return function is already in use */
#define thrd_nomem                                                             \
...
#ifdef _TINYCTHREAD_IMPLEMENTATION
#include <stdlib.h>
...
static void _tinycthread_tss_cleanup(void)
{
    ...
    while (_tinycthread_tss_head != NULL) {
        ...
        _TINYCTHREAD_FREE(_tinycthread_tss_head);
...
#endif
{
    ...
    /* The thread is responsible for _TINYCTHREAD_FREEing the startup
     * information */
    _TINYCTHREAD_FREE((void *)ti);
...
int thrd_create(thrd_t *thr, thrd_start_t func, void *arg)
{
    ...
    _thread_start_info *ti =
        (_thread_start_info *)_TINYCTHREAD_ALLOC(sizeof(_thread_start_info));
    if (ti == NULL) {
        return thrd_nomem;
    }
    ti->mFunction = func;
...
int tss_set(tss_t key, void *val)
{
    ...
    struct TinyCThreadTSSData *data =
        (struct TinyCThreadTSSData *)TlsGetValue(key);
    if (data == NULL) {
        data = (struct TinyCThreadTSSData *)_TINYCTHREAD_ALLOC(
            sizeof(struct TinyCThreadTSSData));
        if (data == NULL) {
            return thrd_error;
        }
