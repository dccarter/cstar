...
static const Type *checkFirstPathElement(AstVisitor *visitor, AstNode *node)
{
    ...
    else if (nodeIs(symbol, GenericDecl)) {
    ...
    if (scope && nodeIs(scope->node, StructDecl)) {
...
void checkPathElement(AstVisitor *visitor, AstNode *node)
{
    ...
    switch (scope->tag) {
    case typContainer:
        env = scope->container.decl->env;
        break;
    case typEnum:
        env = scope->tEnum.decl->env;
        break;
    case typStruct:
        env = scope->tStruct.decl->env;
        break;
    case typModule:
        env = parent->parentScope->env;
        break;
    case typThis:
    ...
    switch (scope->tag) {
    case typEnum:
        if (hasFlag(node->parentScope, Member)) {
            logError(ctx->L,
                     &node->loc,
...
void checkPath(AstVisitor *visitor, AstNode *node)
{
    ...
    if (typeIs(type, Container) && elem->next) {
        elem = elem->next;
        AstNode *symbol = findSymbol(type->container.decl->env,
                                     ctx->L,
