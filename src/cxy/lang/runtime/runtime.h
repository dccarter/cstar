...
#define unreachable(...)                                                       \
    do {                                                                       \
        __cx_assert(false, "Unreachable code reached");                        \
        __builtin_unreachable();                                               \
    } while (0)
#else
#define unreachable(...) __cxy_assert(false, "Unreachable code reached");
#endif
...
typedef struct __cxy_memory_hdr_t {
...
} attr(packed) __cxy_memory_hdr_t;

#define CXY_MEMORY_HEADER_SIZE sizeof(__cxy_memory_hdr_t)
...
static void *__cxy_default_alloc(u64 size, void (*destructor)(void *))
{
    __cxy_memory_hdr_t *hdr = malloc(size + CXY_MEMORY_HEADER_SIZE);
...
static void *__cxy_default_calloc(u64 n, u64 size, void (*destructor)(void *))
{
    __cxy_memory_hdr_t *hdr = calloc(n, size + CXY_MEMORY_HEADER_SIZE);
    hdr->magic = CXY_MEMORY_MAGIC(HEAP);
    hdr->refs = 1;
    hdr->destructor = destructor;
    return CXY_MEMORY_POINTER(hdr);
...
attr(always_inline) void *__cxy_default_get_ref(void *ptr)
{
    if (ptr) {
...
#ifndef __cxy_alloc
#define __cxy_alloc __cxy_default_alloc
#define __cxy_free __cxy_default_dealloc
#define __cxy_realloc __cxy_default_realloc
#define __cxy_calloc __cxy_default_calloc
#endif

#ifndef __builtin_alloc
#define __builtin_alloc(T, n, destructor)                                      \
    __cxy_alloc((sizeof(T) * (n)), (destructor))
#endif

#ifndef __builtin_dealloc
#define __builtin_dealloc(P) __cxy_free((void *)(P))
#endif

#ifndef __builtin_realloc
#define __builtin_realloc(T, P, n, destructor)                                 \
    __cxy_realloc((P), (sizeof(T) * (n)), (destructor))
#endif

#ifndef __builtin_get_ref
#define __builtin_get_ref(P) __cxy_default_get_ref((void *)(P))
#endif

typedef struct __cxy_slice_t {
    u64 len;
    void *data;
    u8 p[0];
} __cxy_slice_t;
...
#define __builtin_alloc_slice(T, n, destructor)                                \
    (T) __cxy_alloc_slice_((n), sizeof(((T)0)->data[0]), (destructor))
#endif

#ifndef __builtin_realloc_slice
#define __builtin_realloc_slice(T, P, n, destructor)                           \
    (T) __cxy_realloc_slice_((P), (n), sizeof(((T)0)->data[0]), (destructor))
#endif
...
attr(always_inline) void __cxy_stack_free(void *ptr)
{
    union {
        void **stack;
        void *ptr;
    } either = {.ptr = ptr};
    __cxy_free(*either.stack);
    *either.stack = NULL;
}

#define __cxy_stack_cleanup __attribute__((cleanup(__cxy_stack_free)))

attr(noreturn) attr(format, printf, 1, 2) void cxyAbort(const char *fmt, ...);

#define __cxy_assert(COND, FMT, ...)                                           \
    if (!(COND))                                                               \
...
static __cxy_stack_str_8_t cxy_wchar_str(wchar chr)
{
    if (chr < 0x80) {
...
{
    __cxy_stack_str_8_t s = cxy_wchar_str(c);
...
attr(always_inline) static void __cxy_string_delete(void *str)
{
    __cxy_string_t *this = str;
    __cxy_free(this->data);
    this->data = nullptr;
    this->data = 0;
}

static __cxy_string_t *__cxy_string_new0(const char *cstr, u64 len)
{
    __cxy_string_t *str =
        __cxy_alloc(sizeof(__cxy_string_t) + len + 1, __cxy_string_delete);
    str->size = len;
    if (cstr != NULL)
...
attr(always_inline) static __cxy_string_t *__cxy_string_new1(const char *cstr)
{
    return __cxy_string_new0(cstr, strlen(cstr));
}

static attr(always_inline)
    __cxy_string_t *__cxy_string_dup(const __cxy_string_t *str)
{
    return __cxy_string_new0(str->data, str->size);
}

static __cxy_string_t *__cxy_string_concat(const __cxy_string_t *s1,
...
static void __cxy_string_builder_delete_fwd(void *sb);
...
attr(always_inline) void __cxy_string_builder_deinit(__cxy_string_builder_t *sb)
{
    if (sb->data)
        free(sb->data);
...
static void __cxy_string_builder_delete_fwd(void *sb)
{
    __cxy_string_builder_delete((__cxy_string_builder_t *)sb);
}
...
attr(always_inline) static void __cxy_string_builder_append_cstr1(
    __cxy_string_builder_t *sb, const char *cstr)
{
    __cxy_string_builder_append_cstr0(sb, cstr, cstr ? strlen(cstr) : 0);
}

attr(always_inline) static void __cxy_string_builder_append_int(
...
attr(always_inline) static void __cxy_string_builder_append_float(
...
attr(always_inline) static void __cxy_string_builder_append_char(
...
attr(always_inline) static void __cxy_string_builder_append_bool(
...
attr(always_inline) static char *__cxy_string_builder_release(
...
typedef uint32_t __cxy_hash_code_t;

attr(always_inline) __cxy_hash_code_t __cxy_hash_fnv1a_init()
{
...
attr(always_inline) __cxy_hash_code_t
    __cxy_hash_fnv1a_uint8(__cxy_hash_code_t h, uint8_t x)
{
...
attr(always_inline) __cxy_hash_code_t
    __cxy_hash_fnv1a_uint16(__cxy_hash_code_t h, uint16_t x)
{
    return __cxy_hash_fnv1a_uint8(__cxy_hash_fnv1a_uint8(h, x), x >> 8);
}

attr(always_inline) __cxy_hash_code_t
    __cxy_hash_fnv1a_uint32(__cxy_hash_code_t h, __cxy_hash_code_t x)
{
    return __cxy_hash_fnv1a_uint16(__cxy_hash_fnv1a_uint16(h, x), x >> 16);
}

attr(always_inline) __cxy_hash_code_t
    __cxy_hash_fnv1a_uint64(__cxy_hash_code_t h, uint64_t x)
{
    return __cxy_hash_fnv1a_uint32(__cxy_hash_fnv1a_uint32(h, x), x >> 32);
}

attr(always_inline) __cxy_hash_code_t
    __cxy_hash_fnv1a_ptr(__cxy_hash_code_t h, const void *ptr)
{
    return __cxy_hash_fnv1a_uint64(h, (ptrdiff_t)ptr);
}

attr(always_inline) __cxy_hash_code_t
    __cxy_hash_fnv1a_string(__cxy_hash_code_t h, const char *str)
{
    while (*str)
        h = __cxy_hash_fnv1a_uint8(h, *(str++));
    return h;
}

attr(always_inline) __cxy_hash_code_t
    __cxy_hash_fnv1a_bytes(__cxy_hash_code_t h, const void *ptr, u64 size)
{
    for (u64 i = 0; i < size; ++i)
        h = __cxy_hash_fnv1a_uint8(h, ((char *)ptr)[i]);
    return h;
}
