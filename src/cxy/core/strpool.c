...
void freeStrPool(StrPool* str_pool) {
    freeHashTable(&str_pool->hash_table);
}
...
const char* makeString(StrPool* str_pool, const char* str) {
    ...
    char** str_ptr =
        findInHashTable(&str_pool->hash_table, &str, hash, sizeof(char*), compareStrs);
    if (str_ptr)
        return *str_ptr;
    ...
    new_str[len] = 0;
    if (!insertInHashTable(&str_pool->hash_table, &new_str, hash, sizeof(char*), compareStrs))
